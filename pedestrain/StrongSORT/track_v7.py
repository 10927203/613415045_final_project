...
from pedestrian.StrongSORT.yolov7.models.experimental import attempt_load
from pedestrian.StrongSORT.yolov7.utils.datasets import LoadStreams, LoadImages
from pedestrian.StrongSORT.yolov7.utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    scale_coords, xyxy2xywh, strip_optimizer, set_logging, increment_path
from pedestrian.StrongSORT.yolov7.utils.plots import plot_one_box
from pedestrian.StrongSORT.yolov7.utils.torch_utils import select_device, load_classifier, time_synchronized, TracedModel

from pedestrian.StrongSORT.strong_sort.utils.parser import get_config
from pedestrian.StrongSORT.strong_sort.strong_sort import StrongSORT



def detect(opt, pre, is_first_image, strongsort, input_save_dir, save_img=False, line_thickness=1):
    opt.save_img = True
    opt.save_txt = True
    source, weights, show_vid, save_txt, imgsz, trace = opt.source, opt.yolo_weights, opt.show_vid, opt.save_txt, opt.img_size, opt.trace
    save_img = not opt.nosave and not source.endswith('.txt')  # save inference images
    ...
    save_crop = False
    project='D:/runs/track'  # save results to project/name
    exp_name='exp'  # save results to project/name
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                #print( 'strongsort_list: ', strongsort_list[i] ) 
                # draw boxes for visualization
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
            ...
            #print(i, '   prev_frames[i]: ', prev_frames[i], ' curr_frames:   ', curr_frames[i] )
            #prev_frames[i] = curr_frames[i]
            return curr_frames[i], strongsort_list, save_dir
...
def yolov7_track_main( input_dir, pre, is_first_image, strongsort, input_save_dir ) :
    parser = argparse.ArgumentParser()
    parser.add_argument('--yolo-weights', nargs='+', type=str, default='./pedestrian/StrongSORT/yolov7/yolov7.pt', help='model.pt path(s)')
    parser.add_argument('--strong-sort-weights', type=str, default = './pedestrian/StrongSORT/weights/osnet_x0_25_msmt17.pt')
    parser.add_argument('--config-strongsort', type=str, default='./pedestrian/StrongSORT/strong_sort/configs/strong_sort.yaml')
    parser.add_argument('--source', type=str, default=input_dir, help='source')  # file/folder, 0 for webcam
    ...
    with torch.no_grad():
        print( input_dir )
        if opt.update:  # update all models (to fix SourceChangeWarning)
            return detect(opt, pre, is_first_image, strongsort, input_save_dir)
            strip_optimizer(opt.weights)
        else:
            return detect(opt, pre, is_first_image, strongsort, input_save_dir)

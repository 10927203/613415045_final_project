...
import cv2
from pedestrian.bounding_box_prediction.utils import *
...
class JAAD(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        self.filename = 'testing.csv'.format(dtype)
        ...
        if(from_file):
            ...
            df = df.apply(lambda row: compute_center(row), axis=1)
            ...
            if save: data.to_csv(os.path.join(self.out_dir, self.filename), index=False)
                
            sequence_centric = data.copy()
            
 
        self.data = sequence_centric.copy().reset_index(drop=False)
...
class JAAD(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        if(from_file):
            ...
            print('sequence_centric: ', sequence_centric)
        ...
        for file in glob.glob(os.path.join(data_dir, dtype,"*")):
            ...
            if not temp.empty:
                ...
                df = pd.concat([df, pd.DataFrame(temp)], ignore_index=True)
        #e_p_read = time.time()
        #print('p_read_csv:', e_p_read-s_p_read)
        #print('Processing data ...')
        #create sequence column
        #s_p_create = time.time()
        ...
        #e_p_create = time.time()
        #print('p_create:', e_p_create-s_p_create)
        
        #d['label'] = d['crossing_true']
        #d.label = d.label.apply(lambda x: 1 if 1 in x else 0)
        
        d = d.drop(d[d.bounding_box.apply(lambda x: len(x) < input )].index)
        d = d.reset_index(drop=True)
        ...
        for i in range(d.shape[0]):
            ...
            k = 0
            #print()
            while (k) <= len(ped.bounding_box) - input:
                ind = np.vstack((ind, ped['ID']))
                bounding_box_o = np.vstack((bounding_box_o, np.array(ped.bounding_box[k:k+input]).reshape(1,input,4)))
                bounding_box_t = np.vstack((bounding_box_t, np.array(ped.bounding_box[k:k+input]).reshape(1,output,4)))
                scene_o = np.vstack((scene_o, np.array(ped.scenefolderpath[k:k+input]).reshape(1,input)))
                file = np.vstack((file, np.array(ped.filename[k:k+input]).reshape(1,input)))
                #cross_o = np.vstack((cross_o, np.array(ped.crossing_true[k:k+input]).reshape(1,input)))
                #cross = np.vstack((cross, np.array(ped.crossing_true[k:k+input]).reshape(1,output)))
        ...
        """
        if save:
            data.to_csv('D:/data/testing.csv', index=False)
        """
